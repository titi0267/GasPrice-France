...
const generateGeoJsonCoordinates = async (start, end) => {
    const generationResult = await (0, request_services_1.default)(`http://router.project-osrm.org/route/v1/driving/${start.split(",")[1] + "," + start.split(",")[0]};${end.split(",")[1] + "," + end.split(",")[0]}?alternatives=2&geometries=geojson&overview=full`, "GET", "osrm");
